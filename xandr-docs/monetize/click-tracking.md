...
> [ !NOTE]
> `${CACHEBUSTER}` macro is a randomly generated string used to prevent URL caching. The value is generated once per bid response, so if a bid request contains multiple `placement_id` values, the same cachebuster value will be used across them. For reconciliation and discrepancy tracking, we recommend using `${AUCTION_ID}` instead.
