...
The Prebid Demand Partner Params Service allows users to view, add, remove, enable, and disable specific partners on PSP configurations. Configurations are managed via the [Config Service](config-service.md). A demand partner must be enabled in the Cross-Partner Settings Service before they can be added to any configurations in the params service and before they can receive bid requests through Prebid Server Premium (PSP). Attempting to add a partner who is not present in the Cross-Partner Settings Service will result in an error.
