...
- Select one of the Segment Reach options for the modeled segment:
    ...
    - **Max Reach:** Leverages Microsoft first-party data and large language models (LLMs) to identify relevant URLs, apps, and contextual environments that align with the seed segment’s characteristics. This approach includes all users in the modeled segment plus additional users reached through contextual expansion.
