...
| `TIMESTAMP` | int | The time at which you would like your segment to go into effect. A UNIX timestamp, down to the *second*. If a timestamp is submitted to greater specificity (down to the millisecond, for example) then the user will not be properly written to the segment upon upload.<br>**Default:** The current time. | `1278250469` |
