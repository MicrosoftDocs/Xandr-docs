...
> [!NOTE]
> **Microsoft Advertising Seller Tags:** When using [Microsoft Advertising's seller tag (AST)](../seller-tag/seller-tag.md), omit the `kw_` prefix. AST has the keyword object in the body of the request, so the prefix is not needed.
...
| Imps (master creative) | The number of page-level roadblocks that served the master creative. <br><br> **Note:** This metric is in alpha testing and not available to all customers. |
