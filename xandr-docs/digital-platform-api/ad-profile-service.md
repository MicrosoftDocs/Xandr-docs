...
| `apply_aq_on_psp` |  boolean | If set to true, checks bids from [Prebid Server Premium](../monetize/prebid-server-premium.md) demand partners against brand and category blocks in the ad profile or `badv` or `bcat` fields of the bid request. Uses `seatbid.bid.adomain` field to infer brand. |
| `reject_unknown_adomain_on_psp` | boolean | Related to `apply_aq_on_psp`. When set to true, rejects bids from [Prebid Server Premium](../monetize/prebid-server-premium.md) demand partners where `seatbid.bid.adomain` values are missing or have not yet been mapped to brands. Also checks bids from [Prebid Server Premium](../monetize/prebid-server-premium.md) demand partners against brand and category blocks in the ad profile or `badv` or `bcat` fields of the bid request. Uses `seatbid.bid.adomain` field to infer brand. |
