...
| `content_season` | string | No | `ptv`, `ssptv`, `vmap`, `ssvmap` | Content season (Example: “Season 3”). This field is important for monetizing CTV (Connected TV) inventory.|
...
| `mimes*` | string | Recommended to monetize . | `ptv`, `ssptv`, `vmap`, `ssvmap` | Video mimes accepted, expressed as a comma-separated list of IDs. You can find the IDs in the [Placement Service documentation](../digital-platform-api/placement-service.md) under "Supported Mime Types."|
...
| `vmin_ad_duration*`| int | Required for monetizing CTV (Connected TV) inventory. | `ptv`, `ssptv`, `vmap`, `ssvmap`| The minimum ad duration, in seconds. |
| `vv` | int | Required for monetizing CTV (Connected TV) inventory. | `ptv`, `ssptv`, `vmap`, `ssvmap` | The VAST version supported by the player:<br> - `2` - VAST 2.0<br> - `3` - VAST 3.0<br> - `4` - VAST 4.0<br> - `5` - VAST 4.1<br> - `6` - VAST 4.2. |
