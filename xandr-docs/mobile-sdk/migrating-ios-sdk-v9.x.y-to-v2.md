...
Download the latest iOS SDK v9.x from [Azure CDN](https://adsdkprod.azureedge.net/mobile/ios/releases/9.11.1/static/sdks.zip). The downloaded zip file contains the following frameworks and resources bundle. Ensure that you are using the appropriate framework that best suits your needs. The **AppNexusSDK** is the recommendation for general use.
