...
|`psp_profile`|object|The `profile` object defines the inventory targeting attributes used to match bid requests to PSP configurations.<br>The profile is either:<br>1. Automatically generated when using the PSP UI, or  <br>2.Created based on user input through the `psp-campaign-objects` API. <br>To modify a profile, use the [`psp-campaign-objects` API](campaign-object-service.md). The configuration service returns the `psp_profile` object in `GET` responses only.|
