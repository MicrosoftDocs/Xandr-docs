...
| Field | Type | Description |
|:---|:---|:---|
| `graph_id` | int | - `Null` if the line item is targeting your member seat's default graph selection. <br> - 0 if the line item is targeting no graph. <br> - `3` if the line item is targeting the TapAd Graph.<br>- `4` if the line item is targeting the Xandr Graph. <br> **Note**: Cross-device audience targeting for CTV devices is now available in the US and EU.|
