...
| Media Type | Yes | Unknown media type refers to bid requests that did not result in impressions. The media type is logged only when a creative is served. For example: `banner`, `video`, `native`. |
...
| Media Type | Unknown media type refers to bid requests that did not result in impressions. The media type is logged only when a creative is served.|
