...
| `apply_aq_on_psp` |  boolean | If set to true [plain text], checks bids from Prebid Server Premium demand partners against brand and category blocks in the ad profile or `badv` [plain text] / `bcat` [plain text] fields of the bid request. Uses `seatbid.bid.adomain [plain text]` field to infer brand. |
| `reject_unknown_adomain_on_psp` | boolean | Related to apply_aq_on_psp [plain text]. When set to true [plain text], rejects bids from [Prebid Server Premium](../monetize/prebid-server-premium.md) demand partners where seatbid.bid.adomain [plain text] values are missing or have not yet been mapped to brands. Also checks bids from [Prebid Server Premium](../monetize/prebid-server-premium.md) demand partners against brand and category blocks in the ad profile or `badv` [plain text] / `bcat` [plain text] fields of the bid request. Uses `seatbid.bid.adomain [plain text]` field to infer brand. |
