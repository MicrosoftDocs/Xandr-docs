...
| `object_type` | string | An optional field for customers to indicate that a segment onboarded by a DMP, or through another of their members, is a first-party segment to them. Segments with `object_type = ‘default’` can be updated to `object_type = ‘first-party’`. This makes a shared segment eligible to be used with Modeled Audiences.<br> **Note**: <br> - By setting the `object_type` to `'first-party'`, you are attesting that your company or a direct client of your company has collected this segment data directly from an individual, or alternatively, your company has licensed this data from a party that collected such data directly from an individual and the terms of your license allow for such data to be used for this look-alike modeling purpose. <br> - Shared segments with `object_type = ‘universal_pixel’` are automatically eligible for Modeled Audiences. <br> - To be eligible for Modeled Audiences, segments must also be composed of Xandr user IDs or MAIDs. Segments targeting URLs or IPs are currently not supported. |
