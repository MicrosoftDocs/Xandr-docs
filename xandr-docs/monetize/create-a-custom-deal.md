...
- **Auction Type** - Select the auction type for the deal.
  ...
  - **Fixed Price** - If you select a fixed-price auction, you will need to enter an ask price. Any buyer who bids above this value will be eligible to bid on the deal. The winning buyer is responsible for the fixed price plus their fees.
    > [!NOTE]
    > The ask price for Fixed Price deals can now only be updated via the API after creation. Please notify the buyer of any price changes, as they will not receive an automated notification. Failing to inform buyers of price increases may halt their line items and impact your revenue flow.
