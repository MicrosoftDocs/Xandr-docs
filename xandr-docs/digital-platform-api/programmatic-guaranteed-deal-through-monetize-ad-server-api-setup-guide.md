...
| `deprioritize_rtb` | boolean | Required | If set to `true`, the PG deal is considered private and always would has priority over open deals and open RTB bids. 
<br>If set to `false`, the PG deal is considered open and competes on price with open deals at the same priority and open RTB bids. <br>Check the [Auction Logic for Deals](../monetize/deal-auction-mechanics.md) for further details.|
...
| `budget_intervals` | array of objects | Required | Always include a `start_date`. Leave `end_date` as `null` for a endless PG deal line item. <br>Here's an [example](#budget_intervals-example) of a budget intervals array setup. |
