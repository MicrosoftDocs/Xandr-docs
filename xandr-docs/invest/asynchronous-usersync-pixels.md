...
description: Learn how a technical upgrade has enabled User syncing to happen after a publisher's page has finished loading and how this will benefit publishers.
ms.date: 10/28/2023
---
...
Due to details of the technical implementation (described below), tools such as [ghostery](http://www.ghostery.com) may show more pixels firing on publishers' pages than before. Users who are fully synced should not see any usersync pixels loading; once they're fully synced, the Ghostery calls will stop.
...
For those interested in more technical details, we are appending a JavaScript callback to the browser's `window.onload` event. For more information about how the `onload` event works, see [Mozilla's DOM Reference](https://developer.mozilla.org/en-US/docs/Web/API/GlobalEventHandlers.onload?redirectlocale=en-US&amp;redirectslug=Web%2FAPI%2Fwindow.onload).
