...
| `supply_type` | string | **Deprecated**. While this field will continue to default to "web", its value is ignored as the supply type is detected automatically by Xandr in each auction. Users must rely on Xandr’s automatic detection rather than specifying the supply type manually. <br><br>**Default**: `web`<br><br>**Note**: **This note is visible to Xandr employees only**: Admins can set this to `facebook_sidebar` as well. 
